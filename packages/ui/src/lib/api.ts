...
export function createErrorResponse(error: Error | ApiError): Response {
  const apiError =
    error instanceof ApiError
      ? error
      : new ApiError(HttpStatus.INTERNAL_SERVER_ERROR, "INTERNAL_SERVER_ERROR", "An unexpected error occurred");
...
export async function parseApiResponse<T, E = unknown>(response: Response): Promise<ApiResponse<T, E>> {
